...
void Select(Tree huffTree[], int *a, int *b, int n)//找权值最小的两个a和b
{
	int i;
...
void Huff_Tree(Tree huffTree[], int w[], string ch[], int n)
{
...
void Huff_Code1(Tree huffTree[], int n)
{
	int i, j, k;
	string s = "";
	std::ofstream ct;
	ct.open("codetable_hw.txt",ios::out|ios::trunc);
	for (int i = 0; i < n; i++)
		{
		s = "";
		j = i;
...
void code1()
{
	...
	fseek(fp,0,SEEK_END);
	long isize = ftell(fp);
	chinese = (unsigned char*)malloc(isize+1);
	rewind (fp);
	fread(chinese,sizeof(char),isize,fp);
	chinese[isize] = '\0';
    ...
    std::fstream pyin;
    pyin.open("pinyin.txt",ios::out|ios::trunc);
	pyin<<pinyin;
	pyin.close();

	string py[pinyin.size()];
...
//  
	...
	Huff_Code1(huffTree, n);
	//输出拼音的编码
    std::ofstream zip;
	zip.open("homeworkzip.txt",ios::out|ios::trunc);
	for(int i = 0 ; i < pinyin.size();i++){
		
        for (int j = 0; j< n; j++){
		if (huffTree[j].ch == py[i]){
			
			cout<<huffTree[j].code;
			zip<<huffTree[j].code;
