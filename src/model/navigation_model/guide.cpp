...
void Guide::print_path_by_time() {
    ...
    int now_build_id;
    int next_build_id;
    string campus_now, campus_next;
    cout << "\n请先输入您当前所在校区以及建筑/教学楼编号(例如 沙河 5): ";
    cin >> campus_now;
    cin >> now_build_id;
    ...
    if (query_hour < 8)    //小于第一节课的时间
        ...
        if (query_minute == 0)
            ...
            class_number = 3;
    } else if (query_hour <= 9) {
        if (query_minute <= 50)
            class_number = 3;
        else
            class_number = 4;
    } else if (query_hour <= 10) {
        if (query_minute <= 40)
            class_number = 4;
        else
            class_number = 5;
    } else if (query_hour <= 11) {
        if (query_minute <= 30)
            class_number = 5;
        else
            class_number = 6;
    } else if ((query_hour <= 12 && query_minute <= 59) || (query_hour == 13 && query_minute == 0)) {
        class_number = 6;
    } else if (query_hour <= 13) {
        if (query_minute <= 50)
            class_number = 7;
        else
            class_number = 8;
    } else if (query_hour <= 14) {
        if (query_minute <= 45)
            class_number = 8;
        else
            class_number = 9;
    } else if (query_hour <= 15) {
        if (query_minute <= 40)
            class_number = 9;
        else
            class_number = 10;
    } else if (query_hour <= 16) {
        if (query_minute <= 35)
            class_number = 10;
        else
            class_number = 11;
    ...
    next_build_id = mp[{query_date, class_number}].second;
    ...
    if (next_build_id == 0) {
        ...
        cout << "\n";
        cout << "当前您所在校区为: " << campus_now << endl;
        cout << "当前所在校区建筑: " << now_build_id << endl;
        cout << "查询课程的校区为: " << campus_next << endl;
        cout << "目的校区建筑编号: " << next_build_id << endl;
        cout << "查询课程名结果为: " << course_name << endl;
        for (int i = 0; i < 3; i++) {
            cout << "..............";
            Sleep(200);
        }
