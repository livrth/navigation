...
void Huff_Code_print(Node huffTree[], int n,string key)
{
	int i, j, k;
	for (i = 0; i < n; i++){
		if (huffTree[i].ch == key){
...
int main()
{
	...
	cout<< "请输入您要转拼音的汉字"<<endl;
	unsigned char chinese[100000];
	cin >> chinese;
	string pinyin;
...
//
	...
	int index = 0;
	for (auto c : dict){
		//cout<<"字母 "<<c.first<<" 出现的频率为: "<< c.second*1.0/dict.size() <<endl;
		str[index] = c.first;
	...
	cout << "哈夫曼编码为：" ;
	for(int i = 0 ; i < pinyin.size();i++){
		Huff_Code_print(huffTree,n,py[i]);
	}
	cout<<endl;
	//解码过程
	string s;
	cout << endl << "解码：" << endl << "输入编码：" ;
	cin >> s;
	cout << Huff_Decode(huffTree, n, s)<< endl;
	string str_check;
	str_check = Huff_Decode(huffTree,n,s);
	if(str_check == pinyin){
		cout << chinese << endl;
	}
	else{
