...
* 除navigation_model外，每个基本模块的代码存储在同名.h以及.cpp文件中，且将相关变量和函数封装在同名的类中。
* 模块依赖关系：
   main.cpp 是程序执行的范围，依赖所有模块。
  student_model依赖 course_model  。navigation_model。
  所有模块依赖menu.cpp. 
...
* 对课程和活动时间进行查询时使用
> 采用理由
* 用户期望在输入一个时间后，能够得到这个时间之后离它最近的那个数据。通过二叉搜索树，这可以在O（ln n） 时间内实现。
...
* 假设数据量为n.
 ...
 两个后续遍历复杂度均为O(ln n)，则只需要O(ln n)复杂度即可得到结果
* 通过insert(x)来添加结点，依然采用后序遍历，则可以在O (ln n)时间内完成活动的添加。
### 3.利用端点和中点坐标的冲突检测算法
 > 场景
